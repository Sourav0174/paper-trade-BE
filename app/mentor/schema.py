...
class MentorResponse(BaseModel):
    """Structured response model generated by AI or Template fallback."""

    headline: str = Field(description="Encouraging headline summary of performance")
    summary: str = Field(description="2-3 sentence overview of trading discipline and health")
    strengths: List[str] = Field(default_factory=list, description="Observed execution strengths")
    mistakes: List[str] = Field(default_factory=list, description="Identified behavioral mistakes")
    risk_warning: Optional[str] = Field(default=None, description="Primary risk alert if applicable")
    action_items: List[str] = Field(default_factory=list, description="Top 1-3 concrete action items")
    motivation: str = Field(description="Positive motivational takeaway")
    next_focus: str = Field(description="Single primary area of focus for upcoming trades")


class DailyMentorReview(BaseModel):
    """Complete daily mentor review containing summary metrics and AI/Template response."""

    user_id: int = Field(description="User identifier")
    review_date: datetime = Field(default_factory=datetime.utcnow, description="Review timestamp")
    summary: MentorSummary = Field(description="Deterministic mentor summary & analytics")
    coaching_response: MentorResponse = Field(description="Structured AI or Template coaching advice")
