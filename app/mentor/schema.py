...
class Insight(BaseModel):
    ...
    coaching_message: str = Field(default="", description="Legacy mentoring feedback")
    action_item: str = Field(default="", description="Legacy recommended step")
...
class RuleEvidence(BaseModel):
    """Factual observation extracted from a triggered rule for AI context."""

    rule_id: str = Field(description="Unique rule identifier")
    title: str = Field(description="Title of detected behavior")
    severity: str = Field(description="Severity impact level")
    metrics_context: dict = Field(default_factory=dict, description="Supporting quantitative facts")


class MentorContext(BaseModel):
    """Compact domain container containing neutral deterministic trading metrics for LLM input."""

    health_score: float = Field(description="Deterministic Trading Health Score (0-100)")
    win_rate_pct: float = Field(description="Win rate percentage")
    total_realized_pnl: float = Field(description="Cumulative net realized profit/loss")
    profit_factor: float = Field(description="Gross profit / Gross loss ratio")
    risk_reward_ratio: float = Field(description="Average profit / Average loss ratio")
    max_drawdown_pct: float = Field(description="Peak-to-trough equity drop percentage")
    holding_duration_ratio: float = Field(description="Ratio of avg losing duration to avg winning duration")
    max_position_sizing_pct: float = Field(description="Largest single position size relative to portfolio")
    total_trades_count: int = Field(description="Total raw execution trades count")
    closed_positions_count: int = Field(description="Total closed round-trip positions count")
...
class PromptContext(BaseModel):
    ...
    system_prompt: str = Field(description="System instructions and guardrails for LLM")
    user_prompt: str = Field(description="Structured JSON context formatted for LLM input")


class MentorResponse(BaseModel):
    """Structured mentor coaching response generated by AI."""

    headline: str = Field(description="Short natural mentor headline")
    mentor_message: str = Field(description="Natural 2-4 paragraph coaching message written directly to the trader")
    key_takeaway: str = Field(description="Single most important lesson")
    risk_warning: Optional[str] = Field(default=None, description="Important risk warning or null")
    next_focus: str = Field(description="Single primary focus area")


class PublicMentorSummary(BaseModel):
    """Clean public summary model excluding internal RuleEngine insights."""

    trading_health_score: float = Field(description="Overall Trading Health Score from 0.0 to 100.0")
    portfolio_summary: dict = Field(description="Quantitative snapshot of performance and health sub-scores")


class DailyMentorReview(BaseModel):
    """Complete daily mentor review containing clean public summary and AI coaching advice."""

    user_id: int = Field(description="User identifier")
    review_date: datetime = Field(default_factory=datetime.utcnow, description="Review timestamp")
    summary: PublicMentorSummary = Field(description="Clean public summary & analytics")
    coaching_response: MentorResponse = Field(description="Structured AI coaching advice")
